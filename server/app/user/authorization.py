...
async def login(db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends(), Authorize: AuthJWT = Depends()):
   ...
   access_token = Authorize.create_access_token(subject = user.name, expires_time=expires)
   ...
   response = {     
        ...
        # Другие нечувствительные данные пользователя, которые вы хотите включить
        }
    }
